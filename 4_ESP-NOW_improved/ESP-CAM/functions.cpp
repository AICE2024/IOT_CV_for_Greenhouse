...
const uint64_t sleepTime = 2 * 60 * 1000000;  // 5 minutes in microseconds
const char* serverName = "https://detect.roboflow.com/xxxxxxxxxxxxxxxxxxxxxxxxxxxx";
...
void goToSleep() {
  ...
  Serial.print("going to sleep for 5min");
  delay(100);
...
float processCamImage() {
    ...
    if (!fb->buf || fb->len == 0) {
        Serial.println("Invalid frame buffer data");
    ...
    return http_robotflow(imageBase64);
}
...
String Photo2Base64(const char* filePath) {
  ...
  // Encode the entire buffer to Base64 in one step
  String imageFile = base64::encode(buffer, fileSize);
...
float processTelloImages() {
  ...
  if (imageCount > 0) {
    ...
    Serial.printf("Average Tello Ripeness: %.2f\n", averageRipeness);
